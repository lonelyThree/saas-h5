...
*
* @param  {[long]} timestamp 时间戳（单位：秒），形如：1550789954
*/
export const formatTimeUnix = function (timestamp) {
...
* @param  {[long]} timestamp 时间戳（单位：秒），形如：1550789954
*/
export const formatTimeUnixCustom = function (timestamp, fmt) {
    if (!timestamp) {
       return '';
    }
...
* @since 1.1
*/
export const getTimeStringAutoShort = function (timestamp, mustIncludeTime = true) {

    // 当前时间
    var currentDate = new Date();
    // 目标判断时间
    var srcDate = new Date(parseInt(timestamp * 1000));
    ...
    if (currentYear == srcYear) {
        ...
        // 当年 && 当天之外的时间（即昨天及以前的时间）
            ...
            // “前天”判断逻辑同上
                ...
                } else if (currentYear == srcYear) {
                    ret = formatDate(srcDate, "M月d日") + timeExtraStr;
                }
                // 否则直接显示完整日期时间
                else
                    ret = formatDate(srcDate, "yyyy年M月d日") + timeExtraStr;
            }
        }
    }
    // 往年
