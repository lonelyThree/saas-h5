<template>
    ...
    <div class="main-content" v-else>
        <div class="rank-bg"
            ...
            <div class="rank-top" flex="dir:left cross:center">
                <div class="top-li" :class="'top-li-'+index" flex="dir:top cross:center" v-for="(item, index) in rankTopList"  :key="item.openid" @click="showDetailClick(item)">
                    ...
                    <div class="time">{{item.costTime || '--'}}s</div>
                </div>
            </div>
            <!-- <div class="rank-div"> -->
            ...
            <van-list
                ...
                <van-cell  v-for="(item, index) in rankList" :key="item.openid">
                    <div class="rank-li"  flex="dir:left cross:center"  @click="showDetailClick(item)">
    ...
    <van-popup
        ...
        class="showDetail"
    >
        <img class="avatar" :src="detailInfo.avatar"  v-if="detailInfo.avatar" />
        <img class="avatar" src="https://vipimg.ai-xbys.com/base/base/rank-avatar.png"  v-else/>
        <div class="name">{{detailInfo.name}}</div>
        <div class="desc">{{detailInfo.chainName}}</div>
        <div class="desc">{{detailInfo.storeName}}</div>


        <div class="score score-num">答题数：<span>{{detailInfo.answerNum || 0}}</span></div>
        <div class="score">总得分：<span>{{detailInfo.score || 0}}</span>分</div>
        <div class="time-div">
            <span class="time"><img src="../../assets/img/time.png" />总用时 {{detailInfo.costTime || '--'}}s</span>
        </div>

        <div class="answer-time">完成时间：{{formatTimeUnixCustom(detailInfo.answerAt, "yyyy年MM月dd日 hh:mm:ss")}}</div>
    </van-popup>
</template>
...
const detailInfo = ref({
    // answerAt: 1697382737,
    // avatar: "https://thirdwx.qlogo.cn/mmopen/vi_32/lNB71eYsgku7XIztugsgibOgEtnBab2zrOX8R1WticzD4LOtOnL7eeicSXNswW46d4HRvgWROhxENVictHT0cqaSulCW6iaMASY36Vp6IpjUw2ss/132",
    // chainName: "老百姓大药房连锁有限公司",
    // costTime: 121,
    // name: "",
    // nickname: "有些幸福 你看不见?",
    // openid: "oblh_swmPqeeAPfLCFd5VNdCpCRs",
    // score: 12,
    // storeName: "长沙南站店"
})

function showDetailClick(item) {
    if(item && item.openid) {
...
function getActivity() {
    ...
    proxy.$axios.get(proxy.$config.domain + 'Activity/GetInfo?type=rank&id=' + aid.value).then(response => {
...
function onLoadRank() {
    ...
    // console.log("domain", proxy.$config.domain)
    // page = 0
    proxy.$axios.get(proxy.$config.domain + 'Activity/GetInfo?type=more&page='+page+'&id=' + aid).then(response => {
        // console.log(response)
        if (response.data.code === 200) {
            const d = response.data.data
            page = page + 1
...
.showDetail {
  ...
  .showDetail   .avatar {
   ...
   .showDetail   .score-num {
        margin-top: 0.5rem;
    }
    .showDetail  .time-div {
        margin-top: 0.2rem;
    }
      .showDetail  .time-div   .time {
            background: #FFAE0025;
