...
function submitInfo() {
    ...
    proxy.$axios.post(proxy.$config.domain + "Activity/AwardAddress", addressInfo.value, { toJSON: true})
    .then(response => {
        // this.showLoading = false;
        if (response.data.code === 200) {
            showToast('保存成功！')
            setTimeout(()=> {
                router.replace({name: routerName, query:{'id':aid, type:aAType}})
...
onMounted(() => {
    ...
    if(id && typeof id === 'string') {
        aid = id
        addressInfo.value.aId = id
    }

    let types = route.query.type
    if(types && typeof types === 'string') {
        aType = types
        addressInfo.value.type = aType
    }
