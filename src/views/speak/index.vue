<template>
    <div class="main-content">
        <div>
            <img src="https://vipimg.ai-xbys.com/saas/img/c8591447/46f869ee/1740316174692.png" />
        </div>
        ...
        <div v-for="(item, index) in list" class="speak-li" :key="item.speakId" @click="speakClick(item)">
            <img :src="item.img">
            <div class="speak-li-btn" flex="dir:left cross:center">
                <img src="../../assets/icons/coins-icon.png" v-if="item.receive_type==3" />
                <img src="../../assets/icons/credit-icon.png" v-else-if="item.receive_type==2" />
                <img src="../../assets/icons/money-icon.png"  v-else />
                <div v-if="item.receive_type==3">星币</div>
                <div v-else-if="item.receive_type==2">积分</div>
                <div v-else>红包</div>
                <div class="money" v-if="item.receive_type==3" >＋{{ item.money/ 100.0  }}</div>
                <div class="money" v-else-if="item.receive_type==2" >＋{{ item.money   }}</div>
                <div class="money"  v-else>￥{{ item.money / 100.0 }}</div>
                <div class="speak-btn" flex="dir:left cross:center main:center">
                    <img src="../../assets/img/speak/audio-icon.png" />
                    <div>答题</div>
                </div>
            </div>
        </div>
...
function getActivity() {
    proxy.$axios.get(proxy.$config.domain + 'Activity/SpeakIndex').then(response => {
...
function speakClick(item) {
    if (item.locFlag) {
        router.replace({ name: 'SpeakSpeak', query: { id: item.speakId, type: 'loc' } })
    } else {
        router.replace({ name: 'SpeakSpeak', query: { id: item.speakId } })
    }
